...
public class LoadingBar : MonoBehaviour
{
    ...
    private IEnumerator FetchPlayerProfileCoroutine()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Get(baseUrl + "/user/profile/view"))
        {
            ...
            SetPlayerNameAndScore(profile.user.firstname);
        }
    }

    // Set the Current Player Name and his Highest Score 
    void SetPlayerNameAndScore(string currentPlayerName)
    {
        string playerFirstname = PlayerPrefs.GetString("PlayerFirstname", "No playerFirstname found");
        string playerMarks = PlayerPrefs.GetString("PlayerMarks", "No playerMarks found");

        if (playerFirstname == "No playerFirstname found" || playerMarks == "No playerMarks found" || playerFirstname != currentPlayerName)
        {
            PlayerPrefs.SetString("PlayerFirstname", currentPlayerName);
